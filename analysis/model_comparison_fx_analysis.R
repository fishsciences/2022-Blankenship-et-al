...
m5.1_hi = readRDS(file.path(res_dir, "modK1.0_tidal01.rds"))

m5.1_med = readRDS(file.path(res_dir, "m0.5_tidal01.rds"))

m5.1_lo = readRDS(file.path(res_dir, "m0.1_tidal01.rds"))
...
write.csv(print(data.frame(lc), digits = 2),
          "results/m5.3_comp.csv", row.names = TRUE)
...
allfx = rbind(cbind(summary(um), Model = "Unidirectional"),
             # cbind(summary(m5.1_med), Model = "October 2019"),
              cbind(summary(K1_disp), Model = "October 2019"),
...
write.csv(allfx, "results/allfx.csv", row.names = TRUE)
...
write.csv(summary(q2), "results/q2_fx.csv")
